...
function custom_contact_form() {
    ...
    if (($_SERVER['REQUEST_METHOD'] == 'POST') && isset($_POST['save'])) {
        ...
        if (!empty($error)) {
            foreach ($error as $error) {
                _e('<span style="color:red">' . $error . '</span></br>');
            }
        } else {
            $contact_form_post = array(
            ...
            $post_id = wp_insert_post($contact_form_post);

            //Add post meta into post_meta
            add_post_meta($post_id, 'first_name', esc_attr($_POST['first_name']), true);
            add_post_meta($post_id, 'description', esc_attr($_POST['description']), true);
            add_post_meta($post_id, 'country', esc_attr($_POST['country']), true);
    ...
    $contact_form = '<form  method="post">		
		...
		<p><input type="text" name="first_name" value = "' . esc_attr($_POST['first_name']) . '" ></p>
		<p><label for="name">Description:</label></p>
		<p><textarea name="description" id="description" rows= "5">' . esc_attr($_POST['description']) . '</textarea>
                <p><label for="country">Country: *</label></p>
		<p><select name="country">
                    <option value="">Select</option>
                    <option value="India" ' . (esc_attr($_POST['country']) == 'India' ? 'selected="selected"' : '') . '>India</option>
                    <option value="USA" ' . (esc_attr($_POST['country']) == 'USA' ? 'selected="selected"' : '') . '>USA</option>
                    </select></p>
                    <p><label for="terms">Terms: *</label></p>
                    <p><input type ="checkbox" name="terms" value ="1" ' . (esc_attr($_POST['terms']) == '1' ? 'checked="checked"' : '') . '> I accept this offer</p>
                    <p><label for="gender">Gender: *</label></p>
                    <p><input type ="radio" name="gender" value ="M" ' . (esc_attr($_POST['gender']) == 'M' ? 'checked="checked"' : '') . '> Male
                       <input type ="radio" name="gender" value ="F" ' . (esc_attr($_POST['gender']) == 'F' ? 'checked="checked"' : '') . '> Female</p>
